...
class ManagementController extends MasterController
{
	...
	public function updateAboutInfos()
	{
		...
		if(!empty($_POST))
		{
			$post = array_map('trim', array_map('strip_tags', $_POST));
			$err=[
				(!v::notEmpty()->alnum('-?!\'*%"ÀÁÂÃÄÅàáâãäåÒÓÔÕÖØòóôõöøÈÉÊËèéêëÇçÌÍÎÏìíîïÙÚÛÜùúûüÿÑñ,._')->length(2, 1000)->validate($post['description'])) ? 'Le champ doit contenir entre 2 et 600 caractères' : null,
